...
Controller.prototype.init = function(callback){
    ...
    if (fs.existsSync('store.json')){
        fs.readFile('store.json',function(err,data){
            ...
            self.epoc.connect('/Users/jpleibundguth/Library/Application Support/Emotiv/Profiles/jleibund.emu', function(e){
              //  console.log(e.time);
                if (e.blink) console.log('BLINK');
                if (e.blink) self.emit(Controller.events.BLINK,e);
                if (e.lookRight) self.emit(Controller.events.LOOK_LEFT,e);
                if (e.lookLeft) self.emit(Controller.events.LOOK_RIGHT,e);
                if (e.winkLeft) self.emit(Controller.events.WINK_LEFT,e);
                if (e.winkRight) self.emit(Controller.events.LOOK_RIGHT,e);
                if (e.cognitivAction == 1 || e.cognitivAction && e.cognitivPower > 0.3) {
                    self.emit(cogMap[e.cognitivAction],e);
//                    console.log('cog', cogMap[e.cognitivAction]);
                }
