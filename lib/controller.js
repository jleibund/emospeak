...
Controller.eventMap = {
    ...
    lookLeft:EventType.LOOK_LEFT,
    lookRight:EventType.LOOK_RIGHT,
    winkLeft:EventType.WINK_LEFT,
    winkRight:EventType.WINK_RIGHT,
    eyebrow:EventType.EYEBROW,
    furrow:EventType.FURROW,
    smile:EventType.SMILE,
    clench:EventType.CLENCH,
    smirkLeft:EventType.SMIRK_LEFT,
    smirkRight:EventType.SMIRK_RIGHT,
    laugh:EventType.LAUGH
};
...
Controller.prototype.say = function(msg, callback){
    if (msg && msg.length >0){
        say.say(msg, callback, {voice:this.ngcm.options.voice});

        //rate can cause an error when specified on systems that have this preset at the OS level prefs
//        say.say(msg, callback, {voice:this.ngcm.options.voice, rate:this.ngcm.options.rate});
...
//    this.favorites = [];
    ...
    self.ngcm.options = this.options || {profile:'', voice:'Ralph',rate:240, throttle:2000, deltaX:9, deltaY:15, action:EventType.BLINK};
    self.dict = spell();
    ...
    if (fs.existsSync('store.json')){
        fs.readFile('store.json',function(err,data){
            ...
            if (parsed.options) self.ngcm.options = parsed.options;
...
Controller.prototype.connect = function(){
    var profile = this.ngcm.options.profile;
    console.log('profile',profile)
    if (profile){
        var self = this;
        var action = self.ngcm.options.action;
        this.epoc.connect(profile, function(e){
            //  console.log(e.time);
//            if (e.blink) {
...
//            }
    ...
    //                }
            ...
            if (e[action]) self.emit(self.eventMap[action],e);
...
Controller.prototype.rezero = function(){
    this.epoc.rezero();
};
...
Controller.prototype.getOptions = function(){
    if (!this.ngcm.options) return null;
    return this.ngcm.options;
}
Controller.prototype.setOptions = function(options,cb){
    if (!this.ngcm.options) return null;
    var recon = this.ngcm.options.profile != options.profile;
    this.ngcm.options = options;
    if (recon) this.reconnect();
    this.store(cb);
}
